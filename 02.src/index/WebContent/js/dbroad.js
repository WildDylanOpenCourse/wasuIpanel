...
var text_0 = "" //文字
var text_1 = ""
	//var c = 0; // 第一次没有拼音框的时候 ，最左边向左移动
	//右边初始化键盘

function dbroad_init() {
		font_Pos = 0;
		font_Pos_1 = 0;
		area_Pos = 10;
		db_Pos = 0;
...
function dbroad_focus(__num) {
		...
		if (shuru == '英文') {
			...
			if (area_Pos == 10) {
				...
				console.log(font_Pos+"!!"+font_Pos_1)
				if (__num == 1) {
					if (font_Pos < font_Pos_1) {
						if (font_Pos == 8 || font_Pos == 17 || font_Pos == 26 || font_Pos == 35) {
...
function font_show() {
		if (shuru == '中文') {
			...
			var a_length = a.length;
			var b = "";
			console.log("a_Pos" + a_Pos + "font_Pos:" + font_Pos + "font_Pos_1:" + font_Pos_1 + "sfz_num:" + sfz_num);
			for (var f = 0; f <= 35; f++) {
				b = a[a_Pos];
				if (b != null) {
		...
		a_Pos = 0;
		font_Pos = 0;
		console.log("222222" + "a_Pos:" + a_Pos + "font_Pos:" + font_Pos + "font_Pos_1:" + font_Pos_1 + "sfz_num:" + sfz_num);
	}
	//文字框翻next页
...
function font_hidden(cl) {
	$(".font_li_" + font_Pos).removeClass("active");
	text_0 = $(".font_li_" + font_Pos).html();
	text_1 = text_1 + text_0;
	console.log("text_0" + text_0 + "text_1" + text_1)
	$("." + cl + "").html(text_1);
	sfz_num = "";
	area_Pos--;
	db_Pos = 0;
