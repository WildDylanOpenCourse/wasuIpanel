 ...
 *
 */
public class NewsGrabPADH {
    
    // 抓新闻的链接
    private String url;// = "http://www.padh.gov.cn/index.php/cms/item-list-category-1.shtml";
    ...
    // 存新闻到数据库的SQL语句
    private static String insertSQL = "insert into padh values (?,?,?,?,?,?)";
     ...
     */
    public NewsGrabPADH(String url, String imgpath, String dbpath, int textlength) {
        this.url = url;
        this.path = imgpath;
        this.dbpath = dbpath;
    ...
    public ArrayList<NewsInfoDto> getNewsList(String strURL) {
        ...
        // 获取有超链接的元素
        Element alist = doc.getElementsByClass("label_ul_b").first();
        Elements newslinks = alist.select("li[style]");
    ...
    public NewsDetailDto getNewsContents(NewsInfoDto info){
        ...
        Element contTxt = doc.getElementById("maincontent");
        // 完整新闻内容
        String fullText;
        // 解析内容时出错 本条新闻不下载
        try {
    ...
    public String setNewsImgFile(Element ele) {
        ...
        File imgFile = new File(path + "\\2\\" + filename);
...
//        
//        NewsGrabPADH hd = new NewsGrabPADH();
//        hd.startgrab();     
//    }
}