 ...
 *
 */
public class NewsGrabZCXX {
    
    // 抓新闻的链接
    private String url;// = "http://www.zhoushan.gov.cn/web/zhzf/zwgk/fgwj/zcjd/index.shtml";
    ...
    // 存新闻到数据库的SQL语句
    private static String insertSQL = "insert into zcxx values (?,?,?,?,?,?)";
     ...
     */
    public NewsGrabZCXX(String url, String imgpath, String dbpath, int textlength) {
        this.url = url;
        this.path = imgpath;
        this.dbpath = dbpath;
    ...
    public ArrayList<NewsInfoDto> getNewsList(String strURL) {
        ...
        // 获取有超链接的元素
        Element alist = doc.getElementsByClass("TYGlR_03").first();
    ...
    public NewsDetailDto getNewsContents(NewsInfoDto info){
        ...
        // 取得网页里属性style中值为font:14px;的元素
        Elements contTxt = doc.getElementsByAttributeValue("style", 
                "font:14px;");
        // 完整新闻内容
        String fullText;
        // 解析内容时出错 本条新闻不下载
        ...
        for (Element link : imgEle) {
            String filename = setNewsImgFile(link, info.getUrl());
            if(!"".equals(filename)) {
                img = img + ";" + filename;
            }
        }
    ...
    public String getNewsFullText(String strHtml){
        ...
        // 从span,p标签中取内容
        Elements eles = doc.select("div>span,div>p");
        for(Element ele : eles) {
          // 有汉汉字并且没有xs0的class才取内容
          Matcher m = p.matcher(ele.text());
          if(m.find() &&
             ele.className().indexOf("xs0") < 0) {
              sb.append(ele.text().replaceAll(" ", " ")+"\n");
          }           
        }
        return sb.toString();
    }
     ...
     * @param ele
     */
    public String setNewsImgFile(Element ele, String parentURL) {

        String filename = ele.attr("oldsrc");
        String strURL = parentURL.substring(0, parentURL.lastIndexOf("/")) 
                                               + "/" + filename;

        InputStream is = null;
        FileOutputStream fos = null;
        File imgFile = new File(path + "\\1\\" + filename);
...
//        
//        NewsGrabZCXX hd = new NewsGrabZCXX();
//        hd.startgrab();     
//    }
}